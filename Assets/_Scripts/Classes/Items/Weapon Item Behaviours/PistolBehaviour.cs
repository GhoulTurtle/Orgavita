...
public class PistolBehaviour : GunWeaponEquippedItemBehaviour{
    ...
    [SerializeField] private FadeObject pistolBulletDecalDent;

    [Header("Fear Pistol Unity Events")]
    [SerializeField] private UnityEvent<AudioSource> OnBulletBounced;

    private PistolWeaponDataSO pistolWeaponDataSO;
    ...
    public override void SetupItemBehaviour(InventoryItem _inventoryItem, PlayerInputHandler _playerInputHandler, PlayerInventoryHandler _playerInventoryHandler){        
        ...
        pistolWeaponDataSO = (PistolWeaponDataSO)weaponData;
    }
    ...
    protected override void HandleGunShotImpact(Ray ray){
        ...
        if(hitInfo.collider.TryGetComponent(out Terrain terrain)){
            ...
            if(rayBounceCounter < pistolWeaponDataSO.maxBounceCount && validBounceableTerrainDataList.IsEnteredTerrainValid(terrainType) && rayBounceCounter < pistolWeaponDataSO.maxBounceCount){
                BounceFearShot(hitInfo, ray);
                //TO-DO: Play spark vfx
                fadeObject = SpawnBulletDecal(pistolBulletDecalDent, hitInfo.point, hitInfo.normal);

                OnBulletBounced?.Invoke(fadeObject.GetFadeObjectAudioSource());
                return;
            }

            if(rayBounceCounter >= pistolWeaponDataSO.maxBounceCount){
                ResetBounceCounter();
            }
